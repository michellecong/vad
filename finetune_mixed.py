...
Fine-tuning strategy:
  ...
  - Mixed training: each batch alternates TORGO loss + LibriParty replay loss
    to prevent catastrophic forgetting of typical speech.
  - REPLAY_WEIGHT controls how much weight the LibriParty replay loss gets.
"""
...
LR           = 1e-4
FREEZE_CNN   = True
REPLAY_WEIGHT = 0.3   # loss = TORGO_loss + REPLAY_WEIGHT * LibriParty_loss
...
SAVE_DIR    = "/courses/CS6140.202630/students/cong.m/results/TinyVAD/loso_mixed"
JSON_DIR    = "/courses/CS6140.202630/students/cong.m/results/VAD/save"
LIBRI_DIR   = "/courses/CS6140.202630/students/cong.m/LibriParty/dataset"
...
# ============================================================
# LibriParty Replay Dataset  (mirrors distill.py)
# ============================================================
class LibriPartyDataset(Dataset):
    """Loads LibriParty JSON for replay during mixed training."""
    def __init__(self, json_path: str):
        with open(json_path) as f:
            self.data = json.load(f)
        self.keys = list(self.data.keys())
        print(f"  LibriParty replay: {len(self.keys)} examples from {json_path}")
    ...
    def __getitem__(self, idx):
        ...
        for seg in item.get("speech", []):
            s = int(seg[0] / (HOP_LENGTH / SAMPLE_RATE))
            e = int(seg[1] / (HOP_LENGTH / SAMPLE_RATE))
...
def finetune_fold(student: TinyVAD,
                  ...
                  save_path: str) -> float:
    """
    Fine-tune student for one LOSO fold with LibriParty replay.
    Each step: loss = TORGO_loss + REPLAY_WEIGHT * LibriParty_loss
    This prevents the model from forgetting typical speech.
    ...
    # Infinite iterator over LibriParty — cycles through regardless of TORGO length
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for mel, labels in train_loader:
            ...
            torgo_loss  = bce(student(mel), labels)

            # Replay: one LibriParty batch per TORGO batch
            r_mel, r_labels = next(replay_iter)
            r_mel, r_labels = r_mel.to(DEVICE), r_labels.to(DEVICE)
            replay_loss = bce(student(r_mel), r_labels)

            loss = torgo_loss + REPLAY_WEIGHT * replay_loss
            optimizer.zero_grad()
            loss.backward()
...
def main():
    ...
    # LibriParty replay loader — shared across all folds
    print("\nLoading LibriParty train set for replay...")
    replay_set    = LibriPartyDataset(os.path.join(JSON_DIR, "train.json"))
    replay_loader = DataLoader(replay_set, batch_size=BATCH_SIZE,
    ...
    for test_spk in ALL_SPEAKERS:
        ...
        best_f1   = finetune_fold(student, train_loader, test_loader, replay_loader, save_path)
